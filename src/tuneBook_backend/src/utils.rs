...
pub async fn update_profile(principal:String, username: String, avatar: Vec<u8>, pob: String, instruments: String) -> types::Profile {
    PROFILE_STORE.with(|profile_store| {
        if profile_store.borrow().get(&principal).is_some(){
...
pub async fn add_tune(principal: String, title: String, tune_data: String, origin: bool, thumbnail: Vec<u8>) -> bool {
    USER_TUNE_STORE.with(|user_tune_store| {
...
pub fn get_friends(principal: String) -> Vec<types::Friend> {
    PROFILE_STORE.with(|profile_store| {
        ...
        if binding.get(&principal).is_some() {
            let friend_principals = profile_store.borrow().get(&principal).unwrap().friends.clone();
            let result: Vec<types::Friend> = friend_principals.iter().map(|friend_principal| {
...
pub async fn accept_friend_request(sender: String, receiver: String) -> bool {
    PROFILE_STORE.with(|profile_store| {
        ...
        if binding.get(&sender).is_some() && binding.get(&receiver).is_some() {
            let mut sender_profile = binding.get(&sender).unwrap().clone();
            let mut receiver_profile = binding.get(&receiver).unwrap().clone();
            let in_position = sender_profile.incoming_fr.iter().position(|ifr| ifr.principal == receiver.clone());
            if in_position.is_some() {
                sender_profile.incoming_fr.remove(in_position.unwrap());
            }
            let out_position = receiver_profile.outcoming_fr.iter().position(|ofr| ofr.principal == sender.clone());
...
pub fn filter_tunes(sub_title: &str, rithm: &str, key: &str, page_num: i32) -> (Vec<types::OriginTune>, i32) {
    TUNE_DB.with(|tune_db| {
        let binding = tune_db.borrow();
        let res: Vec<OriginTune> = binding
        .iter()
        .filter(|(title, tune_data)| 
            title.contains(sub_title) && (rithm == "all" || tune_data.contains(format!("R: {}", rithm).as_str())) && (key == "all" || tune_data.contains(format!("K: {}", key).as_str()))
        )
        .map(|(title, tune_data)| {
        ...
        .collect();
        let result: Vec<OriginTune> = res.iter()
...
pub fn browse_people(filter: String, page_num: i32) -> (Vec<types::Friend>, i32) {
    PROFILE_STORE.with(|profile_store| {
        let res: Vec<Friend> = profile_store.borrow()
        .iter()
        ...
        .collect();
        let result: Vec<Friend> = res.iter()
        .skip(page_num as usize *15 as usize)
        .enumerate()
        .filter(|(index, _)| index.clone() < 15 as usize)
